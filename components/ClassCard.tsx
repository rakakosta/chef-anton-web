...
const ClassCard: React.FC<Props> = ({ item, onViewDetail }) => {
  ...
  return (
    <div className="group bg-white rounded-[2rem] md:rounded-[3rem] overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 border border-stone-100 flex flex-col h-full">
      {/* Container Gambar dengan aspek rasio dinamis */}
      <div className={`relative ${isLive ? 'aspect-[3/4]' : 'aspect-[4/3]'} overflow-hidden m-1.5 md:m-2 rounded-[1.8rem] md:rounded-[2.5rem] bg-stone-100`}>
        <img 
          src={item.image} 
          alt={item.title}
        ...
        <div className="absolute top-3 left-3 md:top-5 md:left-5 flex flex-col gap-1.5 md:gap-2">
          <span className={`px-4 py-1.5 md:px-5 md:py-2 text-[7px] md:text-[8px] font-black tracking-[0.2em] uppercase rounded-full shadow-lg ${
            isLive ? 'bg-red-600 text-white' : 'bg-gold text-white'
          }`}>
            {item.type}
          </span>
          <span className="px-4 py-1.5 md:px-5 md:py-2 bg-white/95 backdrop-blur-sm text-slate-900 text-[7px] md:text-[8px] font-black tracking-[0.2em] uppercase rounded-full shadow-lg border border-stone-100">
            {item.level} Level
          </span>
        </div>
      </div>
      ...
      <div className="p-6 md:p-10 flex flex-col flex-grow">
        <div className="flex-grow">
          <div className="flex items-center gap-2 mb-3">
             ...
             <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{isLive ? 'Live Session' : 'Academy Access'}</span>
          </div>
          <h3 className="text-xl md:text-2xl font-serif text-slate-900 mb-3 md:mb-4 group-hover:text-gold transition-colors leading-tight">
            {item.title}
          </h3>
        ...
        <div className="pt-6 md:pt-8 border-t border-slate-50 flex items-center justify-between mt-auto">
          ...
          <button 
            ...
            className={`px-4 py-2.5 md:px-6 md:py-3 text-white text-[8px] md:text-[10px] font-black uppercase tracking-widest rounded-xl transition-all shadow-md ${
              isLive ? 'bg-slate-900 hover:bg-gold' : 'bg-gold hover:bg-slate-900'
            }`}
          >
