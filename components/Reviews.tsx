...
const Reviews: React.FC<Props> = ({ customReviews }) => {
  ...
  return (
    <section id="reviews" className="py-32 bg-stone-50/50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <span className="text-gold font-black uppercase tracking-[0.3em] text-[10px] mb-4 block">Testimonials</span>
        <h2 className="text-4xl md:text-6xl font-serif text-slate-900 mb-20">Kepercayaan Dari Industri</h2>
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {displayReviews.map((review) => (
            <div key={review.id} className="bg-white p-12 rounded-[3.5rem] border border-stone-100 shadow-xl relative group hover:border-gold/50 transition-all duration-500 text-left">
              ...
              <div className="pt-8 border-t border-stone-50">
                <p className="font-black text-slate-900 text-sm mb-1 uppercase tracking-widest">{review.name}</p>
                <p className="text-[9px] text-gold uppercase tracking-[0.2em] font-black">{review.role}</p>
              </div>
            </div>
          ))}
        </div>
